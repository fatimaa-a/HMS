...
function AdminDashboard() {
  ...
  async function handleApprove(id: number) {
    ...
    try {
      ...
      const approvedUser = await updateUser(token, id, {
        is_active: true,
      });

      setPendingUsers((current) =>
        current.filter((pendingUser) => pendingUser.id !== id)
      );
      ...
      const [updatedDoctors, updatedStaff] = await Promise.all([
        getDoctors(token),
        getStaff(token),
      ]);

      setDoctors(updatedDoctors);
      setStaffCount(updatedStaff.length);
    } catch (error) {
      console.error(error);
